        ...
        with sr.Microphone() as source:
            ...
            while True:
                ...
                if any(word in wake_text for word in ["데브", "고치", "데이브", "대부"]):
                    ...
                    try:
                        requests.post("http://127.0.0.1:5000/api/metrics/log", 
                                      json={
                                          "kind": "voice", 
                                          "name": "wake", 
                                          "ok": True, 
                        ...
                        try:
                            requests.post("http://127.0.0.1:5000/api/metrics/log", 
                                          json={
                                              ...
                                              "request_type": "voice",
                                              "tokens_in": int(len(user_input)/2), # Approx
                                              "tokens_out": token_count,
                                              # Cost calculation (Approx $0.5/1M)
                                              "cost": (token_count / 1000000) * 0.5 
                                          }, timeout=0.5)
                        except Exception as e:
                            console.print(f"[dim]Log Error: {e}[/dim]")
